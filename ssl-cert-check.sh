...
check_server_status() {
    ...
    then
		echo "{ \"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"checkerror\", \"port\": \"${PORT}\", \"checkerror\": \"Connection refused\" } }"
        #prints ${1} ${2} "Connection refused" "Unknown"  
        RETCODE=3

    elif ${GREP} -i "No route to host" ${ERROR_TMP} > /dev/null
    then
		echo "{ \"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"checkerror\", \"port\": \"${PORT}\", \"checkerror\": \"No route to host\" } }"
        #prints ${1} ${2} "No route to host" "Unknown"  
        RETCODE=3

    elif ${GREP} -i "gethostbyname failure" ${ERROR_TMP} > /dev/null
    then
		echo "{ \"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"checkerror\", \"port\": \"${PORT}\", \"checkerror\": \"Cannot resolve domain name with getbyhostname\" } }"
        #prints ${1} ${2} "Cannot resolve domain" "Unknown" 
        RETCODE=3
    
    elif ${GREP} -i "Operation timed out" ${ERROR_TMP} > /dev/null
    then
		echo "{ \"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"checkerror\", \"port\": \"${PORT}\", \"checkerror\": \"Operation timed out\" } }"
        #prints ${1} ${2} "Operation timed out" "Unknown"  
        RETCODE=3
 
    elif ${GREP} -i "ssl handshake failure" ${ERROR_TMP} > /dev/null
    then
		echo "{ \"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"checkerror\", \"port\": \"${PORT}\", \"checkerror\": \"SSL handshake fail\" } }"
        #prints ${1} ${2} "SSL handshake failed" "Unknown"  
        RETCODE=3

    elif ${GREP} -i "connect: Connection timed out" ${ERROR_TMP} > /dev/null
    then
		echo "{ \"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"checkerror\", \"port\": \"${PORT}\", \"checkerror\": \"Timed out\" } }"
...
#        fi
		...
		echo "\"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"Expired\" , \"expires\": \"${CERTDATE}\" , \"daysuntilexpire\" : \"${CERTDIFF}\", \"evilissuer\" : \"${CERTISSUER}\", \"cn\": \"${COMMONNAME}\", \"serial\": \"${SERIAL}\", \"port\": \"${PORT}\" }"
...
#        fi
		...
		echo "\"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"Expiring\" , \"expires\": \"${CERTDATE}\" , \"daysuntilexpire\" : \"${CERTDIFF}\", \"evilissuer\" : \"${CERTISSUER}\", \"cn\": \"${COMMONNAME}\", \"serial\": \"${SERIAL}\", \"port\": \"${PORT}\" }"
        #prints ${HOST} ${PORT} "Expiring" "${CERTDATE}" "${CERTDIFF}" "${CERTISSUER}" "${COMMONNAME}" "${SERIAL}" 
        RETCODE=1

    else

		echo "\"${HOST}:${PORT}\": { \"host\": \"${HOST}\" , \"state\": \"Valid\" , \"expires\": \"${CERTDATE}\" , \"daysuntilexpire\" : \"${CERTDIFF}\", \"evilissuer\" : \"${CERTISSUER}\", \"cn\": \"${COMMONNAME}\", \"serial\": \"${SERIAL}\", \"port\": \"${PORT}\" }"
